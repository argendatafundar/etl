...
# Metadatos 
subtopico <- "SCROLL"
output_name <- "polarizacion_politica_ultimo_anio.csv"
...
df_vdem <- ex$vdem %>% 
  select(geocodigoFundar = country_text_id, anio = year, plarization_index = v2cacamps) 
...
df_paises <- df_vdem %>%  
  ...
  select(anio, geocodigoFundar, geonombreFundar, plarization_index) %>%
  drop_na(plarization_index, geonombreFundar)
...
df_latam <- df_paises %>% 
  ...
  summarise(plarization_index = stats::weighted.mean(plarization_index, population, na.rm = TRUE)) %>% 
  ungroup() %>%
  select(anio, geocodigoFundar, geonombreFundar, plarization_index)
...
valores_referencia <- df_output %>%
    ...
    select(geocodigoFundar, plarization_index)

valor_lcn <- valores_referencia %>% filter(geocodigoFundar == "LCN") %>% pull(plarization_index)
valor_wld <- valores_referencia %>% filter(geocodigoFundar == "WLD") %>% pull(plarization_index)
valor_hic <- valores_referencia %>% filter(geocodigoFundar == "HIC") %>% pull(plarization_index)
valor_arg <- valores_referencia %>% filter(geocodigoFundar == "ARG") %>% pull(plarization_index)

# Encontrar posiciones de las líneas de referencia
pos_lcn <- which(df_plot$plarization_index >= valor_lcn)[1]
pos_wld <- which(df_plot$plarization_index >= valor_wld)[1]
pos_arg <- which(df_plot$plarization_index >= valor_arg)[1]
pos_hic <- which(df_plot$plarization_index >= valor_hic)[1]
...
y_max_log <- max(df_plot$plarization_index) * 1.5

# Crear el gráfico
p <- ggplot(df_plot, aes(x = orden, y = plarization_index)) +
    # Barras
    ...
    geom_text(data = df_plot %>% filter(orden == 1), 
            aes(x = orden , y = plarization_index * 1.1, label = geonombreFundar), 
              hjust = 1.1, vjust = 0.5, color = "black", size = 2, fontface = "bold") +
    geom_text(data = df_plot %>% filter(orden == n()), 
              aes(x = orden + 1, y = plarization_index * 1.1, label = geonombreFundar), 
              hjust = -0.1, vjust = 0.5, color = "black", size = 2, fontface = "bold") +
    # Título y caption
    labs(
        title = paste0("Índice de polarización política, ", anio_grafico),
        x = "",
        y = "",
...
y <- df_vdem %>%   
      inner_join(df_population %>% 
            select( anio = year, geocodigoFundar = iso3c, population = `SP.POP.TOTL`),
             join_by(anio, geocodigoFundar)) %>% 
      drop_na(plarization_index, population) %>%
      group_by(anio) %>%
      summarise(plarization_index = stats::weighted.mean(plarization_index, population, na.rm = TRUE)) %>% 
      ungroup() %>%
      mutate(geocodigoFundar = "WLD", geonombreFundar = "Promedio mundial") %>%
      select(anio, geocodigoFundar, geonombreFundar, plarization_index)


ggplot(y, aes(x = anio, y = plarization_index)) +
        geom_line(color = "#0072ad", linewidth = 0.8) +
        theme_minimal() +
        labs(title = "Índice de polarización política\nPromedio mundial en 1960-2024",
             x = "Año",
             caption = "Fuente de datos: Varieties of Democracy (V-Dem, 2025)",
             y = "Índice de polarización política") +
        ...
        theme(
            ...
            plot.title = element_text(color = "#003c6e", size = 12, face = "bold", hjust = 0),
            plot.caption = element_text(color = "#003c6e", size = 8, hjust = 1),
            axis.text.x = element_text(color = "#a4a4a4", size = 8),
            axis.text.y = element_text(color = "#a4a4a4", size = 8),
            axis.ticks.x = element_line(color = "#a4a4a4"),
            axis.ticks.y = element_line(color = "#a4a4a4"),
        )
