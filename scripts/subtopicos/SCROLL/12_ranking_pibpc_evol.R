...
df_output <- df_prados %>% 
  dplyr::filter(anio >= 1870, variable_name == "Per Capita GDP (Geary-Khamis $1990)") %>% 
  rename(geocodigoFundar = iso3) %>% 
  left_join(geo_front, join_by(geocodigoFundar)) %>% 
  select(anio, geocodigoFundar, geonombreFundar, pib_pc = valor) %>% 
  drop_na(pib_pc) %>% 
  group_by(anio) %>% 
  mutate(ranking = rank(-pib_pc)) %>% 
  ungroup() 


seleccionados <- c("ARG", "ITA", "FIN", "POL", "KOR", "TWN")
...
df_labels <- df_plot %>% 
    dplyr::filter(geocodigoFundar == "ARG", anio %in% c(1870, 1890, 2020))
...
p <- ggplot(df_plot, aes(x = anio, y = ranking, color = geonombreFundar)) + 
    ...
    geom_line() +
    labs(
        title = "Ranking del PIB per cápita de Argentina y países seleccionados, 1870-2020",
        x = "",
        y = "",
