...
df_vdem <- ex$vdem %>% 
  select(geocodigoFundar = country_text_id, anio = year, women_political_empowerment = v2x_gender) 
...
df_paises <- df_vdem %>%  
  ...
  select(anio, geocodigoFundar, geonombreFundar, women_political_empowerment) %>%
  drop_na(women_political_empowerment, geonombreFundar)
...
df_latam <- df_paises %>% 
  ...
  summarise(women_political_empowerment = stats::weighted.mean(women_political_empowerment, population, na.rm = TRUE)) %>% 
  ungroup() %>%
  select(anio, geocodigoFundar, geonombreFundar, women_political_empowerment)
...
df_plot <- df_output %>%
    arrange(women_political_empowerment) %>%
    mutate(
        orden = row_number(),
...
valores_referencia <- df_output %>%
    ...
    select(geocodigoFundar, women_political_empowerment)

valor_lcn <- valores_referencia %>% filter(geocodigoFundar == "LCN") %>% pull(women_political_empowerment)
valor_wld <- valores_referencia %>% filter(geocodigoFundar == "WLD") %>% pull(women_political_empowerment)
valor_hic <- valores_referencia %>% filter(geocodigoFundar == "HIC") %>% pull(women_political_empowerment)
valor_arg <- valores_referencia %>% filter(geocodigoFundar == "ARG") %>% pull(women_political_empowerment)

# Encontrar posiciones de las líneas de referencia
pos_lcn <- which(df_plot$women_political_empowerment >= valor_lcn)[1]
pos_wld <- which(df_plot$women_political_empowerment >= valor_wld)[1]
pos_arg <- which(df_plot$women_political_empowerment >= valor_arg)[1]
pos_hic <- which(df_plot$women_political_empowerment >= valor_hic)[1]
...
y_max_log <- max(df_plot$women_political_empowerment) * 1.5

# Crear el gráfico
p <- ggplot(df_plot, aes(x = orden, y = women_political_empowerment)) +
    # Barras
    ...
    geom_text(data = df_plot %>% filter(orden == 1), 
              aes(x = orden , y = women_political_empowerment * 1.1, label = geonombreFundar), 
              hjust = 1.1, vjust = 0.5, color = "black", size = 2, fontface = "bold") +
    geom_text(data = df_plot %>% filter(orden == n()), 
              aes(x = orden + 1, y = women_political_empowerment * 1.1, label = geonombreFundar), 
              hjust = -0.1, vjust = 0.5, color = "black", size = 2, fontface = "bold") +
    # Título y caption
    labs(
        title = paste0("Índice de empoderamiento político de las mujeres, ", anio_grafico),
        x = "",
        y = "",
